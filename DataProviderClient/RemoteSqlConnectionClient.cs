...
namespace ProductiveRage.SqlProxyAndReplay.DataProviderClient
{
	internal sealed class RemoteSqlConnectionClient : IDbConnection
	{
		...
		public RemoteSqlConnectionClient(IRemoteSqlConnection connection, IRemoteSqlCommand command, IRemoteSqlTransaction transaction, IRemoteSqlDataReader reader, Guid connectionId)
		{
			if (connection == null)
		...
		public IDbCommand CreateCommand() { ThrowIfDisposed(); return new RemoteSqlCommandClient(_connection, _command, _transaction, _reader, _command.GetNewId(ConnectionId)); }
